...
The setup command is the explicit local-consent boundary. It requires server `wordpress-bootstrap` support before copying or activating anything. It may download Miles plugin files into `wp-content/plugins/miles`, or copy a local plugin source only when `MILES_PLUGIN_SOURCE` is explicitly set; when WP-CLI works, it can also activate the plugin, configure `WP_ENVIRONMENT_TYPE=local` for clearly local/dev URLs when application passwords need it, ask the Miles API for local-site credentials, hand those credentials to `wp miles local-setup`, and save the local WordPress admin page as the active Miles surface. WP-CLI is optional: if it is missing or cannot load the local site's database/runtime, the fallback is a user-action handoff, not an agent-click assumption. Ask the user to open `adminPluginsUrl`, sign in to WordPress if prompted, and activate Miles; after they confirm activation, ask them to open `milesAdminUrl` and finish the plugin setup flow. Browser automation may assist when your host has it and the user is already authenticated, but the skill must work without assuming any click capability. Do not write plugin options or secrets yourself. Details: [wordpress-setup.md](references/wordpress-setup.md).
