...
The CLI checks the connected server's supported primitives automatically (`miles doctor --json` shows them under `server.primitives`). If a server-backed primitive is missing there, the server predates it — do not work around it; tell the user. `wordpress-setup --use local` specifically requires `wordpress-bootstrap`; before the server includes that primitive, it exits 2 rather than touching a local WordPress install. Starting a design on a paired local site additionally requires the `site-create.local-wordpress` operation. If it is unavailable, `site-create` exits 2 before uploads, remote mutation, or credit spend; it must never fall through to a cloud site.
