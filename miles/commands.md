...
Agent hosts differ in how they expose live stdout. Codex can run long commands normally because it renders live output in an activity surface. Hosts that buffer foreground stdout, especially Claude Code, should run long Miles commands in the background, tee stdout/stderr to a temp log, and monitor or poll that log for these milestone lines:

```bash
log="${TMPDIR:-/tmp}/miles-$(date +%s)-command.log"
miles create-site "<description>" 2>&1 | tee "$log"
```
