...
`doctor --json` also performs a real create/delete write probe in `MILES_HOME`. When a known host sandbox returns `EPERM`, the `milesHome` check includes `code: "host_sandbox_denied"`, `retryOutsideSandbox: true`, and explicit remediation. Treat that separately from `filesystem_not_writable`, which indicates an ordinary path/permissions problem.
