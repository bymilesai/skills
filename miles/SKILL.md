...
Know this and tell the user when it matters: **a fired run lives on the server, not in your process.** If the user interrupts you (or your host kills a command) mid-run, Miles keeps working — and keeps spending credits. The CLI tracks this: an in-flight marker survives your process, and the next Miles command prints a `[note: ...]` when a previous run may be unsettled (on some hosts a hook injects the same notice automatically). When you see it — or whenever you resume after an interruption — reattach with `miles site-state --json`: the run either finished (use the result) or is still going (`wait-job` to rejoin it, `miles cancel` if the user no longer wants it). Never assume the work died with your process, and never start new Miles work over an unsettled run without telling the user. Details and optional per-host hook setup: [interrupts.md](references/interrupts.md).
