...
The CLI checks the connected server's supported primitives automatically (`miles doctor --json` shows them under `server.primitives`; per-primitive operations are not listed there — the CLI checks them inside the command itself). If a server-backed primitive is missing there, the server predates it — do not work around it; tell the user. When the capabilities check cannot reach the server at all, commands exit 2 with `capabilities_unreachable` and `safeToRetry: true` — a transient outage, not a missing primitive. `wordpress-setup --use local` specifically requires `wordpress-bootstrap`; before the server includes that primitive, it exits 2 rather than touching a local WordPress install. Starting a design on a paired local site additionally requires the `site-create.operations.local-wordpress` operation. If it is unavailable, `site-create` exits 2 before uploads, remote mutation, or credit spend; it must never fall through to a cloud site.
...
The setup command is the explicit local-consent boundary. It requires server `wordpress-bootstrap` support before copying or activating anything. When WP-CLI can report the WordPress version, it inspects the Miles plugin header before copying or activation and an incompatible site exits 2 with detected and required versions; without WP-CLI the version is unknown and the copy proceeds unchecked into the manual handoff. It may download Miles plugin files into `wp-content/plugins/miles`, or copy a local plugin source only when `MILES_PLUGIN_SOURCE` is explicitly set; when WP-CLI works, it can also activate the plugin, configure `WP_ENVIRONMENT_TYPE=local` for clearly local/dev URLs when application passwords need it, ask the Miles API for local-site credentials, hand those credentials to `wp miles local-setup`, and save the local WordPress admin page as the active Miles surface. WP-CLI is optional: if it is missing or cannot load the local site's database/runtime, the fallback is a user-action handoff, not an agent-click assumption. For containerized installs, use the documented official Docker WP-CLI adapter. Ask the user to open `adminPluginsUrl`, sign in to WordPress if prompted, and activate Miles only when automated activation is unavailable; after they confirm activation, ask them to open `milesAdminUrl` and finish the plugin setup flow. Browser automation may assist when your host has it and the user is already authenticated, but the skill must work without assuming any click capability. Do not write plugin options or secrets yourself. Details: [wordpress-setup.md](references/wordpress-setup.md).

Once a local WordPress site is active, it is an exclusive target lock. Treat cloud sites as nonexistent: `sites` returns only that local site, `use` cannot switch away, `site-attach` cannot attach or duplicate another site, and `account-status` does not query or count cloud sites. Continue every build, edit, conversion, export, and browser handoff against the exact paired local site. The only supported exit is an explicit user choice followed by `miles wordpress-setup --use cloud --json`; that clears the active local selection but preserves its saved pairing for later reuse. Never run that command merely to work around a missing local capability.
...
- **"Build me a new website" while another Miles site is active** → do **not** send that prompt through `say` on the current site. Ask one short clarification if intent is ambiguous. On an active paired local WordPress site, `site-create` means start the new design on that exact site; it never creates a cloud fallback. On an active cloud site, it creates a separate deliverable. If they want to redesign an existing cloud site, say that plainly and consider `site-attach <siteId> --duplicate` before risky changes.
...
- **"Edit my Miles site" / "resume where I left off"** → on a cloud connection, `site-attach <siteId>` (cross-machine) or `use <siteId>` (already in local credentials) → `site-state --json` → follow its `next[]` hints. On a paired local WordPress connection, stay on that exact site and run `site-state --json`; cloud attach and switching are intentionally unavailable. When you need the full picture (brief text, what was discussed), add `site-state --full --json` and `history`.
